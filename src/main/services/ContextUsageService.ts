...
class ContextUsageServiceImpl {
  ...
  updateFromStatusLine(ptyId: string, data: RawStatusLinePayload): void {
    // The `shell:` prefix is generated by Dash itself in TerminalDrawer.tsx
    // (`shell:${taskId}[:t${ts}]`) — pure JS, so consistent on every platform.
    // Posts under that id come from a stale `.claude/settings.local.json` that
    // older Dash versions clobbered with a shell ptyId via refreshActivePtyHooks;
    // drop them so we don't grow a ghost entry the renderer has no task row for.
  ...
  private warnStaleShellHook(shellPtyId: string): void {
    // Format guaranteed by TerminalDrawer.tsx: `shell:<taskId>[:t<ts>]`.
    // Warn once per underlying task — repeated posts shouldn't spam toasts.
