...
interface SkillsBrowserBodyProps {
  projects: ProjectInfo[];
  activeProjectId?: string;
  activeTasks: ActiveTaskInfo[];
  currentTaskId?: string;
...
function SkillsBrowserBody({
  projects,
  activeProjectId,
  activeTasks,
  currentTaskId,
}: SkillsBrowserBodyProps) {
  ...
  // Run on mount (so the Installed tab can show a count even before it's opened) and
  ...
  return (
    <>
      {/* Header — same translucent gradient as Settings's sidebar shell so the
          two modals read as siblings stylistically. */}
      <div className="sidebar-shell flex items-center justify-between px-5 h-12 border-b border-border/40 flex-shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          ...
          />
          <span className="text-[13px] font-semibold text-foreground tracking-tight">Skills</span>
        </div>
      ...
      {meta?.status === 'stale' && (
        <div className="flex items-center gap-2 px-4 py-2 border-b border-border/40 bg-destructive/10 text-[11px] text-destructive">
          ...
          <span className="flex-1">
            Refresh failed; showing cached results from {new Date(meta.fetchedAt).toLocaleString()}{' '}
      ...
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-border/30">
        <div className="flex-1 flex items-center gap-2 px-2.5 py-1.5 rounded-lg bg-transparent border border-border/60 focus-within:border-primary/40 transition-colors">
          <Search
            size={ICON_SIZE}
            strokeWidth={ICON_STROKE}
            className="text-muted-foreground flex-shrink-0"
          ...
          <input
            ...
            placeholder={
              view === 'installed'
                ...
                : totalCached > 0
                  ? `Search ${formatCount(totalCached)} skills...`
                  : 'Search skills...'
            }
        ...
        <div className="flex rounded-lg border border-border/60 overflow-hidden text-[12px]">
          ...
          >
            ...
            {installedList.length > 0 && (
              <span
                className={`text-[10.5px] tabular-nums ${view === 'installed' ? 'text-muted-foreground/85' : 'text-muted-foreground/65'}`}
              >
                {formatCount(installedList.length)}
              </span>
            )}
          </button>
        </div>
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-transparent border border-border/60 text-[12px] text-foreground hover:border-primary/40 transition-colors min-w-[120px]"
          >
